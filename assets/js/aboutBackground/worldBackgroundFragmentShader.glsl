...
float top = -0.965;
    float tip = sin(clock * 0.25) * (-0.94 - top) ;
...
void main() {
    float ran = 0.6 * random() * randomOverlay(normalize(vNormal).z);
    if(ran < 0.7){
        ran = 0.0;
    }
    ...
    gl_FragColor = mix(ran * colorCenter, vec4(29.0/255.0,45.0/255.0,68.0/255.0,1.0), 0.5);
